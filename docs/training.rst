...
- :py:class:`.Optimizer`:  a ``lambeq`` optimizer calculates the gradient of a given loss function with respect to the parameters of a model. It contains a :py:meth:`~lambeq.training.optimizer.Optimizer.step` method to modify the model parameters according to the optimizer's update rule. Currently, we support the SPSA algorithm by [Spa1998]_, implemented in the :py:class:`.SPSAOptimizer` class.
